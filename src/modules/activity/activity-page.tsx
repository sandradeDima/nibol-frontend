...
import {
  ...
  type EntityActivityFilters,
} from "@/services/entity-activity-service";
...
const actorLabel = (activity: EntityActivity) =>
  activity.actor?.name ??
...
export function ActivityPage({
  ...
  const queryFilters = useMemo(
    ...
    [canViewTechnical, filters, page, pageSize],
  );
  const activityQuery = useQuery({
    queryFn: () => entityActivityService.list(queryFilters),
    queryKey: [...QUERY_KEYS.entityActivity, queryFilters],
  });
  ...
  const handleExport = async () => {
    ...
    setExportError(null);
    try {
      await entityActivityService.export({
        ...filters,
        includeTechnicalDetails: canViewTechnical,
      });
    } catch {
  ...
  if (activityQuery.isError)
    return (
      <ErrorState
        description="No fue posible cargar la actividad consolidada."
    ...
    <div className="space-y-5">
      <section className="nibol-panel border-t-4 border-t-[var(--primary)] p-5 sm:p-6">
        <div className="grid gap-3 md:grid-cols-2 xl:grid-cols-4">
          <label className="md:col-span-2 xl:col-span-2">
            ...
            <span className="relative block">
              ...
              <input
                className="nibol-field w-full pl-9"
                onChange={(event) => updateFilter("search", event.target.value)}
                placeholder="Código, título, acción o responsable"
                value={filters.search ?? ""}
              />
            </span>
          </label>
          <label>
            <span className="nibol-field-label">Tipo de actividad</span>
            <select
              className="nibol-field w-full"
              onChange={(event) =>
          ...
          <label>
            <span className="nibol-field-label">Entidad</span>
            <select
              className="nibol-field w-full"
              onChange={(event) =>
          ...
          <label>
            <span className="nibol-field-label">Origen</span>
            <select
              className="nibol-field w-full"
            ...
            >
              ...
              <option value="SYSTEM">Sistema / cron</option>
            </select>
          </label>
          <label>
            <span className="nibol-field-label">Usuario</span>
            <select
              className="nibol-field w-full"
              onChange={(event) =>
                updateFilter("actorUserId", event.target.value)
              }
          ...
          <label>
            <span className="nibol-field-label">Área</span>
            <select
              className="nibol-field w-full"
              onChange={(event) => updateFilter("areaId", event.target.value)}
              value={filters.areaId ?? ""}
            >
              <option value="">Todas</option>
              {(configurationQuery.data?.areas ?? []).map((area) => (
          ...
          <label>
            <span className="nibol-field-label">Desde</span>
            <input
              className="nibol-field w-full"
          ...
          <label>
            <span className="nibol-field-label">Hasta</span>
            <input
              className="nibol-field w-full"
        ...
        <div className="mt-5 flex flex-wrap items-center justify-between gap-3 border-t border-stone-200 pt-4">
          <p className="text-sm text-stone-600">
            {pagination.total} eventos visibles según su alcance de
            autorización.
          </p>
          <div className="flex flex-wrap gap-2">
            <button
              className={
                view === "timeline"
                  ? "nibol-btn-primary px-3 py-2 text-sm"
                  : "nibol-btn-secondary px-3 py-2 text-sm"
              }
              onClick={() => setView("timeline")}
              type="button"
            >
              <List className="h-4 w-4" /> Línea de tiempo
            </button>
            <button
              className={
                view === "table"
      ...
      <section className="nibol-panel p-5 sm:p-6">
        {activityQuery.isPending ? (
          <p className="py-8 text-sm text-stone-500">Cargando actividad…</p>
        ) : activities.length === 0 ? (
          ...
          <div className="overflow-x-auto">
            <table className="min-w-full text-left text-sm">
              <thead>
                <tr className="border-b border-stone-200 text-xs tracking-[0.14em] text-stone-500 uppercase">
              ...
              <tbody>
                {activities.map((activity) => (
                  <tr
                    ...
                    key={
                      activity.id ?? `${activity.createdAt}-${activity.title}`
                    }
                  >
                    ...
                    <td className="px-3 py-4 text-stone-700">
                      {activity.observation
                        ? `${activity.observation.code} — ${activity.observation.title}`
                        : "—"}
                    </td>
                    <td className="px-3 py-4 text-stone-700">
                      {actorLabel(activity)}
                    ...
                    <td className="px-3 py-4 text-stone-600">
                      {activity.actorType === "USER" ? "Usuario" : "Sistema"}
                    </td>
                    <td className="px-3 py-4">
                      {activity.targetUrl ? (
                        <Link
