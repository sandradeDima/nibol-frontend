...
const labelByKey: Record<string, string> = {
  ...
  overdue_activity_enabled: "Registrar vencimientos en la trazabilidad",
  pending_extension_reminder_hours:
    "Recordar aprobaciones de ampliación después de (horas)",
  pending_review_reminder_hours:
    "Recordar revisiones pendientes después de (horas)",
...
const formatDate = (value: string | null) =>
  value ? new Date(value).toLocaleString("es-BO") : "Sin ejecución";
...
export function AutomaticNotificationAdmin({
  ...
  const executionsQuery = useQuery({
    queryFn: () =>
      notificationService.listJobExecutions({ page: 1, perPage: 8 }),
      ...
      <section className="grid gap-4 md:grid-cols-3">
        <div className="nibol-panel p-5">
          ...
          <p className="mt-5 text-lg font-semibold text-[var(--foreground)]">
            {formatDate(latest?.finishedAt ?? null)}
          </p>
          <p className="mt-2 text-sm text-[var(--muted)]">
            {latest?.status ?? "Aún no ejecutado"}
          </p>
        </div>
        <div className="nibol-panel p-5">
          <div className="flex items-center justify-between gap-3">
            <span className="nibol-eyebrow">Alertas creadas</span>
            <ShieldAlert className="h-5 w-5 text-[var(--accent)]" />
          </div>
          <p className="mt-5 text-3xl font-semibold text-[var(--foreground)]">
            {latest?.notificationsCreated ?? 0}
          </p>
          <p className="mt-2 text-sm text-[var(--muted)]">
        ...
        <div className="nibol-panel p-5">
          <div className="flex items-center justify-between gap-3">
            <span className="nibol-eyebrow">Correos enviados</span>
          ...
          <p className="mt-2 text-sm text-[var(--muted)]">
            Las fallas se registran por entrega
          </p>
        </div>
      </section>
      ...
      <div className="grid gap-6 xl:grid-cols-[1.15fr_0.85fr]">
        <section className="nibol-panel p-6">
          <div className="flex flex-wrap items-start justify-between gap-4">
            <div>
              <p className="nibol-eyebrow">Reglas operativas</p>
              <h2 className="mt-2 text-2xl font-semibold text-[var(--foreground)]">
                Vencimientos y recordatorios
              </h2>
              <p className="mt-2 max-w-2xl text-sm leading-6 text-[var(--muted)]">
                El monitor conserva los estados de revisión y solo actualiza
                estados vencidos cuando el flujo actual lo permite.
              </p>
            </div>
            {canExecute ? (
              <button
          ...
          <div className="mt-6 divide-y divide-[var(--border)] border-y border-[var(--border)]">
            {rules.map((rule) => {
              ...
              return (
                <div
                  className="grid gap-4 py-4 md:grid-cols-[minmax(0,1fr)_12rem_auto] md:items-center"
                  key={rule.key}
                >
                  <div>
                    <p className="font-semibold text-[var(--foreground)]">
                      {labelByKey[rule.key] ?? rule.name}
                    </p>
                    <p className="mt-1 text-xs leading-5 text-[var(--muted)]">
                      {rule.description ??
                        "Parámetro de automatización con valor seguro por defecto."}
                    </p>
                    <code className="mt-2 inline-block text-[11px] text-[var(--primary)]">
                      {rule.key}
                    </code>
                  </div>
                  {booleanRule ? (
                  ...
                  <button
                    ...
                    disabled={value === rule.value || updateMutation.isPending}
                    onClick={() =>
                      void updateMutation.mutateAsync({ key: rule.key, value })
                    }
                    type="button"
                  >
                    Guardar
        ...
        <section className="nibol-panel p-6">
          ...
          <div className="mt-6 flex items-center gap-2 text-xs leading-5 text-[var(--muted)]">
            <CheckCircle2 className="h-4 w-4 shrink-0 text-[var(--success)]" />
            La ejecución automática no guarda secretos ni cuerpos privados de
            correo.
          </div>
        </section>
      </div>
